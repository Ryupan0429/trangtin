...
router.get('/', async (req, res) => {
	...
	var bv = await BaiViet.find({KiemDuyet: 1}).sort({ ngaydang: -1 }).populate('ChuDe').populate('TaiKhoan').limit(12).exec();
	
	var xnn = await BaiViet.find({KiemDuyet: 1}).sort({ ngaydang: -1 }).populate('ChuDe').populate('TaiKhoan').limit(3).exec();
	res.render('index', {
		title: 'Trang chủ',
		chuyenmuc: cm,
		baiviet: bv,
...
router.get('/baiviet/chude/:id', async (req, res) => {
	...
	// Lấy chuyên mục hiện thị vào menu
	var cm = await ChuDe.find();
